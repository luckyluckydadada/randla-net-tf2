...
class Network:
    ...
    def inference(self, inputs, is_training):
        ...
        feature = helper_tf_util.conv2d(
            f_encoder_list[-1],
            f_encoder_list[-1].get_shape()[3].value,
            [1, 1],
            "decoder_0",
            [1, 1],
            "VALID",
    ...
    def train(self, dataset):
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                _, _, summary, l_out, probs, labels, acc = self.sess.run(
                ...
                if self.training_step % 50 == 0:
                    message = (
                        "Step {:08d} L_out={:5.3f} Acc={:4.2f} " "---{:8.2f} ms/batch"
                    )
                    log_out(
                        message.format(
                            self.training_step, l_out, acc, 1000 * (t_end - t_start)
                        ),
                        self.Log_file,
                    )
                self.training_step += 1
                ...
                log_out(
                    "Best m_IoU is: {:5.3f}".format(max(self.mIou_list)), self.Log_file
                )
                ...
                op = self.learning_rate.assign(
                    tf.multiply(
                        self.learning_rate, self.config.lr_decays[self.training_epoch]
                    )
                )
                self.sess.run(op)
                log_out("****EPOCH {}****".format(self.training_epoch), self.Log_file)
    ...
    def evaluate(self, dataset):
        ...
        for step_id in range(self.config.val_steps):
            ...
            try:
                ...
                stacked_prob, labels, acc = self.sess.run(
                    ops, {self.is_training: False}
                )
    ...
    def relative_pos_encoding(self, xyz, neigh_idx):
        ...
        xyz_tile = tf.tile(
            tf.expand_dims(xyz, axis=2), [1, 1, tf.shape(neigh_idx)[-1], 1]
        )
